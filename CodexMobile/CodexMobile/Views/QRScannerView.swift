...
struct QRScannerView: View {
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                if let command = prompt.command, !command.isEmpty {
                    ...
                    bridgeUpdateStep(number: "1", title: "Update App", detail: "Install the latest Harmony build on this iPhone.")
    ...
    private var scannerOverlay: some View {
        VStack(spacing: 24) {
            ...
            Text("Scan the Harmony QR code")
                .font(AppFont.subheadline(weight: .medium))
                .foregroundStyle(.white)

            pairingFallbackButtons
    ...
    @ViewBuilder
    private var pairingFallbackButtons: some View {
        VStack(spacing: 12) {
            if let onManualCode {
                scannerSecondaryButton("Pair with Code", action: onManualCode)
            }
        }
    }

    private func scannerSecondaryButton(_ title: String, action: @escaping () -> Void) -> some View {
    ...
    private func handleScanResult(_ code: String, resetScanLock: @escaping () -> Void) {
        ...
        case .success(let payload):
            onScan(payload)
        case .shortCode:
            scannerError = "Use Pair with Code below."
            resetScanLock()
...
private extension CodexBridgeUpdatePrompt {
    static let previewScannerMismatch = CodexBridgeUpdatePrompt(
        title: "Update the bridge on your Mac before scanning",
        message: "This QR code was generated by a different bridge version. Update the package on your Mac to the latest release before scanning a new QR code.",
